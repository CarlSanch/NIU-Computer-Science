...
//
//
//	Programmer's Note: I have an issue where I am not getting the correct addresses showing up in dump.
...
string rv32i_decode::decode(uint32_t addr, uint32_t insn)
{
	...
	switch(get_opcode(insn))
	{
		...
		case opcode_stype:
			...
			{
				default: return render_illegal_insn(insn);
				case funct3_sb: return render_itype_load(insn, "sb");
				case funct3_sh: return render_itype_load(insn, "sh");
				case funct3_sw: return render_itype_load(insn, "sw");
			}
			assert(0 && "unrecognized funct3"); //impossible
