...
private:
	...
	binTree <T>::binTree()
	{
		root = nullptr; // Need to create the root for the functions
	}
	...
	{
		insert(root, value); // calls private version of insert
	}

	template <typename T>
	void binTree<T>::inorder(void (*temp) (const T &value)) const
	{
	...
	template<class T>
	void binTree<T>::inorder(Node <T> *node, void(*print)(const T &value))
	{
		if(node != nullptr)
		{
